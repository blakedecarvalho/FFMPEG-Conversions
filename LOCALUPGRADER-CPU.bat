...
for /r "%cd%" %%a in (*.mp4 *.avi *.mkv *.flv) do (
    ...
    echo "!file!" | findstr /i /c:"_upscaled" >nul && (
        ...
        if not exist "!upscaled!" (
            ...
            echo "!file!"
            ffmpeg -i "!file!" -vf scale=3840x2160:flags=bicubic -rc constqp -qmin 17 -qmax 51 -qp 24 -preset veryslow -rc-lookahead 4 -profile:v high -bf 0 -keyint_min 1 -refs 7 -qdiff 20 -qcomp 0.9 -me_method umh -c:v libx264 -async 1 -threads 4 -c:a copy "!upscaled!" -loglevel quiet
